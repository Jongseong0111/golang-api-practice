...
const getSignedId = `-- name: GetSignedId :one
SELECT user_id FROM scm_account
WHERE user_account=?
`

func (q *Queries) GetSignedId(ctx context.Context, userAccount string) (int32, error) {
	row := q.db.QueryRowContext(ctx, getSignedId, userAccount)
	var user_id int32
